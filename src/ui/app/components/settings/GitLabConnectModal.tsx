...
import { Gitlab, Copy, Check, Loader2, ExternalLink, AlertCircle, CheckCircle } from 'lucide-react';
...
export function GitLabConnectModal({ workspace, onClose, onSuccess }: GitLabConnectModalProps) {
  ...
  const startDeviceFlow = async () => {
    ...
    try {
      const response = await api<DeviceFlowResponse>(
        'POST',
        `/workspaces/${workspace.id}/gitlab/connect`
      );
      ...
      const message = err instanceof Error ? err.message : 'Failed to start GitLab connection';
      setError(message);
      setStage('error');
    }
  };
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        const response = await api<PollResponse>(
          'GET',
          `/workspaces/${workspaceId}/gitlab/status`
        );
  ...
  return (
    <Modal
      ...
      title={stage === 'success' ? 'Connected!' : `Connect GitLab to "${workspace?.name}"`}
    >
      <div className="space-y-4">
        ...
        {stage === 'loading' && (
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <p className="text-sm text-zinc-400">Initializing GitLab connection...</p>
          </div>
        )}
          ...
          <>
            <div className="space-y-3">
              <p className="text-sm text-zinc-400">
                1. Go to <span className="font-medium text-zinc-200">gitlab.com/-/profile/device</span>
              </p>
              <button
          ...
          <>
            ...
            <div className="text-center space-y-1">
              <p className="text-xs text-zinc-400">
                Git push, pull, and merge request creation now work automatically for repositories in this workspace.
              </p>
              <p className="text-xs text-zinc-500">
                No SSH keys or GitLab CLI needed.
              </p>
            </div>

            <button
              onClick={handleDone}
              className="w-full rounded-xl bg-orange-600 py-2.5 text-sm font-medium text-white transition-colors hover:bg-orange-500"
            >
              Done
            </button>
          </>
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-orange-600 py-2.5 text-sm font-medium text-white transition-colors hover:bg-orange-500"
              >
                Try Again
              </button>
            </div>
