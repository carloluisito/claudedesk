...
interface RunConfigurationCardProps {
  repoId: string;
  onStarted: () => void;
}

export function RunConfigurationCard({ repoId, onStarted }: RunConfigurationCardProps) {
  ...
  // Load monorepo info and Docker info on mount
  useEffect(() => {
    if (repoId) {
      detectServices();
      loadDockerStatus();
  ...
  return (
    <GlassCard padding="lg">
      ...
      <div className="space-y-4">
        {/* Loading state */}
        {isDetecting ? (
          <div className="flex items-center justify-center gap-2 rounded-2xl bg-white/5 ring-1 ring-white/10 py-6">
            <Loader2 className="h-5 w-5 animate-spin text-white/40" />
            <span className="text-sm text-white/50">Detecting services...</span>
          </div>
        ) : isMonorepo ? (
          /* Monorepo Services Panel */
          <div className="rounded-2xl ring-1 ring-purple-500/30 bg-purple-500/5">
            <div className="flex items-center gap-2 border-b border-purple-500/20 px-3 py-2.5">
              <Layers className="h-4 w-4 text-purple-400" />
              <span className="text-sm font-medium text-white">Detected Services</span>
            ...
            <div className="p-3 space-y-2">
              {monorepoInfo!.services.map((service) => {
                ...
                return (
                  <div
                    key={service.id}
                    className="flex items-center justify-between rounded-xl bg-white/5 ring-1 ring-white/10 px-3 py-2"
                  >
          ...
          <>
            <div>
              ...
              <input
                ...
                min={1}
                max={65535}
                className="w-full rounded-2xl bg-white/5 ring-1 ring-white/10 px-3 py-2 text-sm text-white focus:ring-white/20 focus:outline-none"
                disabled={isStarting}
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-white/70 mb-1.5">
                Run Command <span className="text-white/40">(optional)</span>
              </label>
              <input
                type="text"
                value={command}
                ...
                className="w-full rounded-2xl bg-white/5 ring-1 ring-white/10 px-3 py-2 text-sm text-white placeholder-white/30 focus:ring-white/20 focus:outline-none"
                disabled={isStarting}
              />
            </div>
          </>
        ...
        <div>
          <button
            ...
            className="flex w-full items-center justify-between rounded-2xl bg-zinc-800/50 px-3 py-2.5 ring-1 ring-white/10"
          >
            <div className="flex items-center gap-2">
              <Settings className="h-4 w-4 text-white/40" />
              <span className="text-sm font-medium text-white/70">Environment Variables</span>
              {envVars.length > 0 && (
            ...
            <ChevronDown
              className={cn('h-4 w-4 text-white/40 transition-transform', showEnvVars && 'rotate-180')}
            />
          </button>

          {showEnvVars && (
            <div className="mt-2 space-y-2 rounded-2xl bg-zinc-800/50 ring-1 ring-white/10 p-3">
              {envVars.map((envVar, index) => (
                <div key={index} className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="KEY"
                    className="w-1/3 rounded-xl bg-white/5 ring-1 ring-white/10 px-2 py-1.5 text-xs font-mono text-white placeholder-white/30 focus:ring-white/20 focus:outline-none"
                  />
                  ...
                  <input
                    ...
                    placeholder="value"
                    className="flex-1 rounded-xl bg-white/5 ring-1 ring-white/10 px-2 py-1.5 text-xs font-mono text-white placeholder-white/30 focus:ring-white/20 focus:outline-none"
                  />
                  <button onClick={() => removeEnvVar(index)} className="p-1 text-white/40 hover:text-red-400">
        ...
        <div>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-white/70">Docker Options</span>
              {dockerEnabled && (
                <span className="rounded-md bg-blue-500/20 px-1.5 py-0.5 text-xs text-blue-400">Enabled</span>
              )}
            </div>
            <ChevronDown
              className={cn('h-4 w-4 text-white/40 transition-transform', showDockerOptions && 'rotate-180')}
            />
          </button>

          {showDockerOptions && (
            <div className="mt-2 space-y-3 rounded-2xl bg-zinc-800/50 ring-1 ring-white/10 p-3">
              {/* Docker status warning */}
              {dockerStatus && !dockerStatus.available && (
                <div className="rounded-xl ring-1 ring-yellow-500/30 bg-yellow-500/10 px-2.5 py-2 text-xs text-yellow-400">
              ...
              <label className="flex items-center justify-between">
                <span className="text-sm text-white/70">Enable Docker</span>
                <button
                  onClick={() => setDockerEnabled(!dockerEnabled)}
                  disabled={!dockerStatus?.available}
                  className={cn(
                    'relative h-6 w-11 rounded-full transition-colors',
                    dockerEnabled ? 'bg-blue-600' : 'bg-white/10',
              ...
              {dockerEnabled && (
                <>
                  <div>
                    <label className="mb-1 block text-xs text-white/50">Image Name (optional)</label>
                    <input
                      type="text"
                      value={dockerImageName}
                      onChange={(e) => setDockerImageName(e.target.value)}
                      placeholder={`claudedesk-${repoId}`}
                      className="w-full rounded-xl bg-white/5 ring-1 ring-white/10 px-2 py-1.5 text-xs text-white placeholder-white/30 focus:ring-white/20 focus:outline-none"
                    />
                  </div>

                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="mb-1 block text-xs text-white/50">Memory Limit</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="mb-1 block text-xs text-white/50">CPU Limit</label>
                      <input
                        type="text"
        ...
        <div className="rounded-2xl bg-zinc-800/50 ring-1 ring-white/10 p-3">
          <label className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Cloud className={cn('h-4 w-4', tunnelEnabled ? 'text-orange-500' : 'text-white/40')} />
              <div>
                <span className="text-sm font-medium text-white/70">Cloudflare Tunnel</span>
            ...
            <button
              ...
              className={cn(
                'relative h-6 w-11 rounded-full transition-colors',
                tunnelEnabled ? 'bg-orange-500' : 'bg-white/10'
              )}
            >
              <span
                className={cn(
        ...
        {/* Error display */}
        {error && (
          <div className="flex items-start gap-2 rounded-2xl bg-red-500/10 ring-1 ring-red-500/30 p-3">
        ...
        <button
          ...
          className={cn(
            'flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-semibold text-white transition',
